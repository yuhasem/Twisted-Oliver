...
public class Control {
	...
	/**
	 * Initializes the game (later we will implement deck choice)
	...
	public static void main(String[] args) {
		...
		int turn = 1;
		while (player1.getCollection() < Player.MAX_COLLECTION && player2.getCollection() < Player.MAX_COLLECTION) {
			System.out.println("\n\t\tTURN: " + turn + "\t\tPLAYER: "
		...
		while (line == "") {
			...
			if (line.equals("play")) {
				...
				if (toPlay >= 0 && toPlay < adjacent.inHand.size()) {
					...
					if (moving.getType().charAt(0) == 'i' && moving.getType().charAt(1) == 'o' && orbplay){
							System.out.println("You have already played an orb");
					} else if (moving.getType().charAt(0) == 'i' && moving.getType().charAt(1) == 'e') {
						System.out.println("Which Battler do you want to attach it to?");
						int toAttach = -1;
						if (input.hasNextInt()) {
						...
						if (toAttach >= 0 && toAttach < adjacent.inPlay.size()) {
							...
							if (attach.getType().charAt(0) == 'b'){
								...
								attaching.onEquipEffect(attach2, adjacent, opposing);
							} else {
								System.out.println("Invalid card: not a Battler");
							}
						} else {
							System.out.println("Invalid number: not in Play size.");
						}
					} else {
						...
						if (moving.onEnterEffect(adjacent, opposing)){
							...
							if (moving.getType().charAt(0) == 'i' && moving.getType().charAt(1) == 'o'){
								orbplay = true;
							}
						} else {
				...
				System.out.println("Which card? [use the numbers after card listed in hand]");
				int toView = -1;
				if (input.hasNextInt()) {
				...
				System.out.println("Which card? [use the numbers after card listed in play]");
				int toView = -1;
				if (input.hasNextInt()) {
					toView = input.nextInt();
				}
				...
				System.out.println("Which card? [use the numbers after card listed in opponent's play]");
				int toView = -1;
				if (input.hasNextInt()) {
					toView = input.nextInt();
				}
			...
			if (line.equals("call")) {
				...
				if (toAtt >= 0 && toAtt < adjacent.inPlay.size()) {
					if (adjacent.inPlay.get(toAtt).getType().charAt(0) == 'b'){
						Battler attacking = (Battler) adjacent.inPlay.get(toAtt);
						//choose target
						int toTar = -2;
						System.out.println("Which card to Target? [use the numbers after card listed in opponent's play]");
						if (input.hasNextInt()){
							toTar = input.nextInt();
						}
						if (toTar >= -1 && toTar < opposing.inPlay.size()){
							if (toTar == -1){
								if (!opposing.hasAttackers()){
									adjacent.getConnectedPlayer().changeCollection(1);
									attacking.called = true;
								}
							} else if (opposing.inPlay.get(toTar).getType().charAt(0) == 'b'){
								Battler defending = (Battler) opposing.inPlay.get(toTar);
								attacking.attack(defending, adjacent, opposing);
							} else {
								System.out.println("Invalid card: not a Battler");
							}
						} else {
							System.out.println("Invalid entry: Number not within Player's hand size");
						}
					} else {
						System.out.println("Invalid card: not a Battler");
					}
				}
				if (adjacent.getConnectedPlayer().getCollection() > Player.MAX_COLLECTION || opposing.getConnectedPlayer().getCollection() > Player.MAX_COLLECTION){
					return true;
				}
				line = "";
			} else if (line.equals("proceed")) {
